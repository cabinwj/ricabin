...
{
    status_frame::m_run_status_.set(run_status_reload);
#ifndef WIN32
...
{
    status_frame::m_run_status_.set(run_status_exit);
#ifndef WIN32
...
#ifdef WIN32
static const char* xml_relative_path[] = { "statusd.xml", "online_data", };
#else
static const char* xml_relative_path[] = { "/statusd.xml", "/online_data", };
#endif
...
bitmap32 status_frame::m_run_status_;
...
int status_frame::start(bool is_daemon)
{
    if (is_daemon)
    {
        init_daemon(1, 0);
        cout << "status_frame::start() daemon start!" << endl;
    }
    ...
    {
        cout << "status_frame::start() get current work path (getcwd) error!" << endl;
        exit(0);
    }
    cout << "status_frame::start() current work path: " << work_path << endl;
#endif

    strncpy(xml_full_path, work_path, strlen(work_path));
    strncat(xml_full_path, xml_relative_path[0], strlen(xml_relative_path[0]));

    xml_configure& statusxml = CREATE_XML_CONFIG_INSTANCE();
    statusxml.load_all(xml_full_path);

    LOG_INIT(statusxml.m_net_xml_[statusxml.m_use_index_].m_log_.m_log_path_,
             statusxml.m_net_xml_[statusxml.m_use_index_].m_log_.m_file_size_,
             statusxml.m_net_xml_[statusxml.m_use_index_].m_log_.m_log_level_);

    LOG_OPEN();

    // 初始化底层框架
    int rc = init_environment(statusxml.m_net_xml_[statusxml.m_use_index_].m_common_.m_entity_id_);
    if (0 != rc)
    {
        LOG(ERROR)("status_frame::start() init environment fail.");
        return -1;
    }
    LOG(INFO)("status_frame::start() coroutine frame actor is running.");
    ...
    memcpy(nconf.m_ipport_.m_ip_,
           statusxml.m_net_xml_[statusxml.m_use_index_].m_listen_.m_ip_port_.m_ip_,
           sizeof(nconf.m_ipport_.m_ip_));
    nconf.m_ipport_.m_port_ = statusxml.m_net_xml_[statusxml.m_use_index_].m_listen_.m_ip_port_.m_port_;
...
void status_frame::on_timer()
{

}

void status_frame::on_tick()
{
    //检查dbagent 断线重连
