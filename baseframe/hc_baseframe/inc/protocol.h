...
enum proto_t
{
    PROTOCOL_IN = (int16_t)0,    //异步操作协议(包括请求、应答、普通包) （仅限服务器之间通讯S/S）
    PROTOCOL_EX = (int16_t)1,    //二进制流协议(C/S, B/S, T/S)
} ;
...
//public:
    ...
    net_event_callback_t m_net_event_cb_;

    uint32_t m_socket_send_buffer_size_; // socket 发送缓冲区大小
    uint32_t m_socket_recv_buffer_size_; // socket 接收缓冲区大小

    uint32_t m_connect_out_uin_;         // 仅对连出去的socket有效: 连出去的服务标识，对端的服务器id

    int16_t m_connect_out_timeout_;      // 仅对连出去的socket有效：建立连结的超时时间（单位：s）
    int16_t m_keepalive_interval_;       // 仅对连出去的socket有效: 心跳包，设置为0无心跳包（单位：s） 

    int16_t m_keepalive_timeout_;        // 多久没收到心跳包（或任何包），做断开处理（单位：s）
