...
net_id_guard::net_id_guard()
{
    ...
    for (uint32_t id = TCP_ACCEPTOR_BEGIN; id <= TCP_ACCEPTOR_END; id++) {
        m_tcp_acceptor_id_queue_->push(id);
    }

    m_tcp_stream_id_queue_ = new net_id_queue( TCP_STREAM_END - TCP_STREAM_BEGIN + 1 );
    for (uint32_t id = TCP_STREAM_BEGIN; id <= TCP_STREAM_END; id++) {
...
uint32_t net_id_guard::acquire(net_id_guard::net_id_t type)
{
    ...
    switch (type)
    {
    case net_id_guard::ID_TCP_CONNECTOR:
        rc = m_tcp_connector_id_queue_->pop(id, true);
        break;
    case net_id_guard::ID_TCP_ACCEPTOR:
        rc = m_tcp_acceptor_id_queue_->pop(id, true);
        break;
    case net_id_guard::ID_TCP_STREAM:
